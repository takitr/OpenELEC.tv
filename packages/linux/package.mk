...
case "$LINUX" in
  amlogic)
    PKG_VERSION="amlogic-3.10.33-9eb2bde"
...
post_patch() {
  if [ -n "$BOARD" -a -f $PROJECT_DIR/$PROJECT/$PKG_NAME/$PKG_NAME.$TARGET_ARCH.$BOARD.conf ]; then
    KERNEL_CFG_FILE=$PROJECT_DIR/$PROJECT/$PKG_NAME/$PKG_NAME.$TARGET_ARCH.$BOARD.conf
